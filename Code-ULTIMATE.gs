...
function getMenuData() {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      // ✅ Supporta sia vecchio campo "giorno" che nuove colonne giorni (LUN..DOM)
      const giorni = getGiorniFromRow(row, prodotto.giorno);
      addProductToMenu(menu, prodotto, giorni);
    }
...
function aggiungiProdotto(data) {
  ...
  const giorni = resolveGiorniPayload(data);
  ...
  const newRow = [
    ...
    'SI',
    giorni.lunedi ? 'SI' : 'NO',
    giorni.martedi ? 'SI' : 'NO',
    giorni.mercoledi ? 'SI' : 'NO',
    giorni.giovedi ? 'SI' : 'NO',
    giorni.venerdi ? 'SI' : 'NO',
    giorni.sabato ? 'SI' : 'NO',
    giorni.domenica ? 'SI' : 'NO'
  ];
  ...
  try {
    ...
    for (let i = 1; i < dataRange.length; i++) {
      if (dataRange[i][0] == data.id) {
        ...
        // ✅ Salva giorni come SI/NO nelle colonne 11-17
        const giorni = resolveGiorniPayload(data);
        Logger.log('📅 Salvataggio giorni:');
        sheet.getRange(i + 1, 11).setValue(giorni.lunedi ? 'SI' : 'NO');
        sheet.getRange(i + 1, 12).setValue(giorni.martedi ? 'SI' : 'NO');
        sheet.getRange(i + 1, 13).setValue(giorni.mercoledi ? 'SI' : 'NO');
        sheet.getRange(i + 1, 14).setValue(giorni.giovedi ? 'SI' : 'NO');
        sheet.getRange(i + 1, 15).setValue(giorni.venerdi ? 'SI' : 'NO');
        sheet.getRange(i + 1, 16).setValue(giorni.sabato ? 'SI' : 'NO');
        sheet.getRange(i + 1, 17).setValue(giorni.domenica ? 'SI' : 'NO');
        Logger.log('   LUN: ' + (giorni.lunedi ? 'SI' : 'NO'));
        Logger.log('   MAR: ' + (giorni.martedi ? 'SI' : 'NO'));
        Logger.log('   MER: ' + (giorni.mercoledi ? 'SI' : 'NO'));
        Logger.log('   GIO: ' + (giorni.giovedi ? 'SI' : 'NO'));
        Logger.log('   VEN: ' + (giorni.venerdi ? 'SI' : 'NO'));
        Logger.log('   SAB: ' + (giorni.sabato ? 'SI' : 'NO'));
        Logger.log('   DOM: ' + (giorni.domenica ? 'SI' : 'NO'));
...
function resolveGiorniPayload(data) {
  if (data && data.giorni) {
    return {
      lunedi: !!data.giorni.lunedi,
      martedi: !!data.giorni.martedi,
      mercoledi: !!data.giorni.mercoledi,
      giovedi: !!data.giorni.giovedi,
      venerdi: !!data.giorni.venerdi,
      sabato: !!data.giorni.sabato,
      domenica: !!data.giorni.domenica
    };
  }

  return mapGiornoToGiorni(normalizzaGiorno(data && data.giorno ? data.giorno : 'OGGI'));
}

function mapGiornoToGiorni(giorno) {
  const giorni = {
    lunedi: false,
    martedi: false,
    mercoledi: false,
...
function getGiorniFromRow(row, giornoRaw) {
  const giorniColonne = [row[10], row[11], row[12], row[13], row[14], row[15], row[16]];
  const hasBooleanDays = giorniColonne.some(value => value === 'SI' || value === 'NO');
  ...
  return mapGiornoToGiorni(normalizzaGiorno(giornoRaw));
}

function addProductToMenu(menu, prodotto, giorni) {
  const dayMap = {
    0: 'sunday',
    1: 'monday',
    2: 'tuesday',
  ...
  if (giorni.lunedi) menu.weekly.monday.push(prodotto);
  if (giorni.martedi) menu.weekly.tuesday.push(prodotto);
  if (giorni.mercoledi) menu.weekly.wednesday.push(prodotto);
  if (giorni.giovedi) menu.weekly.thursday.push(prodotto);
  if (giorni.venerdi) menu.weekly.friday.push(prodotto);
  if (giorni.sabato) menu.weekly.saturday.push(prodotto);
  if (giorni.domenica) menu.weekly.sunday.push(prodotto);

  if (todayKey && giorni[dayNameToField(todayKey)]) {
    menu.today.push(prodotto);
  }
}

function dayNameToField(dayName) {
  const map = {
    monday: 'lunedi',
    tuesday: 'martedi',
    wednesday: 'mercoledi',
